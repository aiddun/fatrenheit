...
mod fat_sample {
    ...
    impl FatSample {
        ...
        #[ink(message)]
        pub fn weather(&mut self) -> Result<String, Error> {
            let mut headers: Vec<(String, String)> = Vec::new();

            let header: (String, String) = ( 
                String::from("User-Agent"),  
                String::from("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/102.0.5005.63 Safari/537.36 Edg/102.0.1245.33")
            );
            
            headers.push(header);
            let response = http_get!("https://api.weather.gov/gridpoints/EWX/155,91/forecast", headers);

            let temp = extract_temp(&response.body);
    ...
    fn extract_temp(body: &[u8]) -> Result<String, Error> {
        let body: String = String::from_utf8_lossy(body).chars().collect();
        // let pos = body.find("\"temperature\": ").ok_or(Error::NoClaimFound)?;
        // // let end = body.find("\"temperature\": ").ok_or(Error::NoClaimFound)?;
